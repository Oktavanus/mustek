...
while($data=mysql_fetch_array($has)){
  $art = substr($data['isi_jadwal'],0,100);
   echo '
          ...
          <img width="800" height="480" src="photo-jadwal/'.$data['photo_jadwal'].'">
          <h6>Dipublikasikan pada '.$data['tgl_jadwal'].'</h6>
          <h6>Oleh '.$data['penulis_jadwal'].'</h6>
          <p>'.$art.'..</p>
